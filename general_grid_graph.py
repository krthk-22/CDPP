...
NUM_H_TILES = int(input("Enter the width of the grid: ")) # number of horizontal tiles
NUM_V_TILES = int(input("Enter the height of the grid: ")) # number of vertical tiles
WINDOW_H_SIZE = NUM_H_TILES * TILE_SIZE # width of the window
WINDOW_V_SIZE = NUM_V_TILES * TILE_SIZE # height of the window

# Initialize the display
screen = pygame.display.set_mode((WINDOW_H_SIZE, WINDOW_V_SIZE))
pygame.display.set_caption("Graph Game")
3
font = pygame.font.Font(None, 36) 

dxn = ['up', 'right', 'left', 'down'] # directions
move_num = {'up': 0, 'right': 1, 'left': 2, 'down': 3} # direction to number
key_binds = "asdfghjkl" # key binds
custom = {} # custom key binds

'''
i, j on grid => horizontal_length*i + j + 1
cell no c => c/horizontal_length, (c - 1)%horizontal_length
graph[c] = list of cell nos blank can move to from c  
'''

def to_num(i, j):
    return NUM_H_TILES*i + j + 1 # changed 4 to NUM_TILES

def to_rc(c):
    c = int(c)
    i = int((c-1)/NUM_H_TILES) 
    j = (c-1)%NUM_H_TILES 
    return i, j
...
class Puzzle15:
    def __init__(self):
        self.board = [[i + j * NUM_H_TILES for i in range(1, NUM_H_TILES + 1)] for j in range(0, NUM_V_TILES)]
        self.empty_row, self.empty_col = NUM_V_TILES - 1, NUM_H_TILES - 1
    ...
    def get_possible_moves(self):
        ...
        if curr_cell + NUM_H_TILES in self.graph[curr_cell]:
    ...
    def get_state(self):
        l = [i for i in range(1, NUM_H_TILES*NUM_V_TILES + 1)]
        perm = []
        isVisit = [0 for i in range(NUM_H_TILES*NUM_V_TILES)] 
        for c in range(1, NUM_H_TILES*NUM_V_TILES + 1):
            i, j = to_rc(c)
            l[c-1] = self.board[i][j]

        for i in range(NUM_H_TILES*NUM_V_TILES): 
            if l[i] == i+1:
                isVisit[i] = 1
    ...
    def draw_board(self):
        for row in range(NUM_V_TILES):
            for col in range(NUM_H_TILES):
                ...
                if num == NUM_H_TILES*NUM_V_TILES and (row + col) % 2 == 0: 
                    pygame.draw.rect(screen, ORANGE, (col * TILE_SIZE, row * TILE_SIZE, TILE_SIZE, TILE_SIZE))
                elif num == NUM_H_TILES*NUM_V_TILES and (row + col) % 2 == 1:
                    ...
                    text_rect = text.get_rect(center=(col * TILE_SIZE + TILE_SIZE // 2, row * TILE_SIZE + TILE_SIZE // 2))
                    screen.blit(text, text_rect)
        for cell in range(1, 1 + NUM_H_TILES*NUM_V_TILES): 
            i, j = to_rc(cell)
            ...
            for adj in self.graph[cell]:
                ...
                endX -= dx/NUM_H_TILES
                endY -= dy/NUM_V_TILES
                draw_arrow(startX + dx/NUM_H_TILES, startY + dy/NUM_V_TILES, endX, endY, (255, 0, 0))
    ...
    def set_bind(self):
        ...
        custom[curr_key] = [NUM_H_TILES*NUM_V_TILES]
